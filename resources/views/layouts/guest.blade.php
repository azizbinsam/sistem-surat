...
<head>
    ...
    <title>{{ \App\Models\AppSettings::current()->nama_aplikasi }}</title>
    ...
    <div class="min-h-screen flex">
        ...
        <div class="hidden lg:flex lg:w-1/2 bg-zinc-900 flex-col justify-between p-12">
            <a href="{{ route('welcome') }}" class="text-white font-bold text-xl">
                <x-app-logo class="text-white font-bold text-xl" />
            </a>
            <div>
                <h2 class="text-white text-3xl font-bold leading-tight mb-4">
                    Generate Surat NPB, SPB, SPPB<br>Dalam Hitungan Detik.
                </h2>
                <p class="text-zinc-400">Upload Excel, sistem urus penomoran, ledger stok, sampai dokumen siap tanda
                    tangan.</p>
            </div>
            <p class="text-zinc-600 text-sm">&copy; {{ date('Y') }}
                {{ \App\Models\AppSettings::current()->nama_aplikasi }}</p>
        </div>
        ...
        <div class="flex-1 flex items-center justify-center p-6 bg-zinc-50">
            <div class="w-full max-w-sm">
                <div class="lg:hidden mb-8 text-center">
                    <x-app-logo class="font-bold text-xl text-zinc-900" />
                </div>
